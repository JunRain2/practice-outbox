  ...
  └─ @Transactional
      ...
      └─ Outbox 저장 (Event) + ApplicationEvent 발행
          └─ [커밋 후] EventListener (@Async)
              └─ EventProcessor.process() ── Redis 분산 락
```
...
```
OutboxEventScheduler (@Scheduled) ── 글로벌 분산 락 (outbox)
  └─ EventExecutor.execute()
      └─ PENDING 이벤트 조회 (grace period 적용)
          └─ EventProcessor.process() ── 개별 분산 락 (event_id:{id})
```
...
- **글로벌 락 (`outbox`)**: 이벤트가 많이 쌓인 상태에서 다중 인스턴스의 스케줄러가 동시에 실행되면 불필요한 DB 조회가 발생한다. 글로벌 락으로 스케줄러를 단일 실행으로 제한.
- **개별 락 (`event_id:{id}`)**: 동일 이벤트의 중복 처리 방지. Listener와 Scheduler가 같은 이벤트를 동시에 처리하는 것을 차단.

### Scheduler → Executor → Processor 분리

관심사 분리:
- **Scheduler**: 언제 실행할지 (fixedRate + 글로벌 락)
...
outbox:
  scheduler:
    ...
    lease-seconds: 30        # 글로벌 스케줄러 락 유지 시간
  retry:
    default:                 # 기본 재시도 전략
      max-count: 5
      base-seconds: 60
    strategies:              # 이벤트 타입별 재시도 전략
      MOCK_CREATED:
        max-count: 5
        base-seconds: 60
```
...
| **EventProcessorTest** | 이벤트 처리 성공/실패/재시도, 지수 백오프 적용, 5회 초과 실패 시 FAILED 마킹, 동일 이벤트 동시 처리 시 1건만 성공, 서로 다른 이벤트 병렬 처리 |
